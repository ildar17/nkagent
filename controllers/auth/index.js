    ...
    Auth.getHashUrl(req.params.hashEmail.trim(), function (err, hashUrl) {
      ...
      if (!hashUrl) {
        ...
        if (hashUrl.date_hash_url > Date.now()) {
          ...
          res.render('auth/rebuild', {title: 'Новый пароль', h1: 'Восстановление пароля. Новый пароль', email: hashUrl.email});
...
exports.recoveryForm = function (req, res) {
  ...
  if (req.session.uid) {
    ...
    res.render('auth/recovery', {title: 'Восстановление пароля', h1: 'Заявка на восстановление пароля', total: captcha.data});
...
exports.registerForm = function (req, res) {
  ...
  if (req.session.uid) {
    ...
    res.render('auth/register', {title: 'Регистрация', h1: 'Регистрация', total: captcha.data});
...
exports.loginForm = function (req, res) {
  ...
  if (req.session.uid) {
    ...
    res.render('auth/login', {title: 'Авторизация', h1: 'Общегородской прайс риелторов г.Нижнекамск', total: captcha.data});
