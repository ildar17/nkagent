  ...
  pool.connect( function(err, client, done) {
    ...
    client.query('DELETE FROM temporarily_users_userdata WHERE id_user = $1', [user.id_user], function (err, result) {
      ...
      pool.connect(function (err, client, done) {
        ...
        client.query('INSERT INTO temporarily_users_userdata (email, now_date, url_hash, id_user, fio, tel, agency, note) ' +
          'VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
          [user.email, user.now_date, user.url_hash, user.id_user, user.fio, user.tel, user.agency, user.note], function (err, result) {
            done();
            if (err) return fn(err, null, null);
