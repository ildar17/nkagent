...
Complete.list = function (agent, fn) {
  ...
  pool.connect(function (err, client, done) {
    ...
    client.query("SELECT id AS Редактирование, (SELECT title FROM node WHERE id = type) AS Тип, title AS Заголовок, date_create AS ДатаВнесения, date_edit AS ДатаПравки, (SELECT (SELECT title FROM city WHERE id_city = s.city_id) FROM street s WHERE s.id_street = c.street) AS Город, (SELECT s.street FROM street s WHERE s.id_street = c.street) AS Улица, price AS Цена, note AS Примечание, main AS Главная, status AS Публик FROM complete c WHERE template IN('apartment','cottages','commercial') AND agent = $1 ORDER by template, section, title, Улица", [agent], function (err, result) {
      done();
      if (err) return fn(err, null);
